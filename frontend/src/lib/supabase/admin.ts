...
export async function fetchPendingApplications() {
  ...
  // Attach short-lived signed URLs so Admin's "View" buttons work without
...
/**
 * Admin creates a supervisor account. Since Supabase auth requires a
 * password at signup and admins shouldn't be setting passwords on someone
 * else's behalf, this uses `inviteUserByEmail` via a Supabase Edge Function
 * running with the service_role key (never exposed to the browser). See
 * `backend/edge-functions/invite-supervisor.ts` for that function; this is
 * the client-side call that triggers it.
 */
...
/**
 * Admin/supervisor resolves a dispute by either releasing escrow to the
 * runner or refunding it to the customer. Wrap this in its own RPC (like
 * accept_quote / approve_and_release in schema.sql) if you want it

 *
 *   create function public.resolve_dispute(p_task_id uuid, p_refund boolean) ...
 *
 * mirroring approve_and_release but crediting the customer's wallet instead
 * when p_refund is true.
 */
